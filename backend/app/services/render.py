...
# # Shorter segments lip-sync far more accurately on Seedance (less room for the
# # model to drift / hallucinate words over a long clip), at the cost of more clips.
# TARGET_SEGMENT_MS = 8_000    # Higgsfield clip target (~8s for tight lip-sync)
# MIN_SEGMENT_MS = 5_000       # avoid very short clips when possible
# HARD_MAX_MS = 11_000         # never exceed
# MIN_TAIL_MS = 3_000          # merge tiny leftovers into previous segment
...
#     """Speaker-bounded segmentation. Each segment belongs to EXACTLY ONE speaker
#     (a segment never spans a host->guest boundary). A single long turn is
#     sub-split at internal silences to stay near the 10-12s target; short turns
#     stay whole. Returns (start_ms, end_ms, speaker)."""
#     segments: list[tuple[int, int, str]] = []
#     for (t_start, t_end, _text, speaker) in turn_spans:
#         turn_len = t_end - t_start
#         if turn_len <= TARGET_SEGMENT_MS:
#             segments.append((t_start, t_end, speaker))
#             continue
#         # long turn: sub-split at internal silences, greedily near target
#         cands = _detected_silences_within(final, t_start, t_end)
#         cur = t_start
#         while t_end - cur > TARGET_SEGMENT_MS:
#             limit = cur + TARGET_SEGMENT_MS
#             preferred = [c for c in cands if cur + MIN_SEGMENT_MS <= c <= limit]
#             fallback = [c for c in cands if cur < c <= limit]
#             stretch = [c for c in cands if limit < c <= cur + HARD_MAX_MS]
#             if preferred:
#                 cut = max(preferred)
#             elif fallback:
#                 cut = max(fallback)
...
#                 cut = min(stretch)
#             else:
#                 cut = _quietest_point(final, limit)
#             if t_end - cut <= MIN_TAIL_MS:
#                 break
#             segments.append((cur, cut, speaker))
#             cur = cut
#         segments.append((cur, t_end, speaker))  # remainder of this turn
#     if not segments:
