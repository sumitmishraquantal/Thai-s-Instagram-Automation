...
def find_by_token(token: str) -> tuple[dict, str] | None:
    """Read-only lookup: return (record, owner) for a token WITHOUT resolving it.
    Used by the review page and the per-scene rerun so that reviewing or
    regenerating clips does NOT consume/resolve the approval."""
    with _LOCK:
        store = _read()
        for rec in store.values():
            for o, t in rec.get("owner_tokens", {}).items():
...
def update_payload(approval_id: str, payload: dict) -> dict | None:
    """Replace a pending record's payload (e.g. after regenerating some clips) so a
    re-sent email reflects the new clips. No-op if the record is gone."""
    with _LOCK:
        store = _read()
        ...
        if not rec:
            return None
        rec["payload"] = payload
        rec.setdefault("history", []).append({"action": "payload_updated", "at": _now()})
        _write(store)
        return rec
